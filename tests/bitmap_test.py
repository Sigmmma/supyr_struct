...
'''Generate the 24 bit R8G8B8 pixels for the bitmap'''
...
for y in range(256):
    i = y*bytes_per_line
    y = 255-y  # invert the value because bmp images are upside-down
...
#####################  BMP  #####################
'''Put the dimensions and bpp into the header, set
the pixels pointer, the size of the file, put the
pixels into the tag, and write the bmp to a file.'''
...
bmp_tag.serialize(temp=False, backup=False)
...
dds_tag.serialize(temp=False, backup=False)
...
tga_tag.serialize(temp=False, backup=False)
...
indexing = bytearray(b'\x08\xff\x00\xfd\xf9\xeb\xc7o\x9f\xbe|\xf8\xee\
...
for i in range(256):
    ...
    image.image_data_wrapper.image_data = indexing

gif_tag.serialize(temp=False, backup=False)
